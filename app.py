...
options = [{'label': 'All Sites', 'value':'ALL'},{'label':sites[0],'value':sites[0]},{'label':sites[1],'value':sites[1]},{'label':sites[2],'value':sites[2]},{'label':sites[3],'value':sites[3]}]
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df,values='class',
                     names = 'Launch Site',
                     title='Total Success ratio on All Sites')
        return fig    
    else:
        data = spacex_df[spacex_df['Launch Site'] == entered_site] 
        fig = px.pie(data,
                     names='class',
                     title=f'Proportion of Success and Failure on {entered_site} site') 
        return fig
# TASK 4:
...
def get_scatter_chart(entered_site,payload_range):
    ...
    if entered_site == 'ALL':
        data = spacex_df[(spacex_df['Payload Mass (kg)' ]>= min_value) & 
